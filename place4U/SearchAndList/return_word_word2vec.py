...
def get_word2vec(entire_Tag_list, recent_search_word):
    #words = "짬뽕, 아메리카노,아메리카노, 짜장면, 유린기, 탕수육, 삼선짜장, 삼선짬뽕, 볶음밥, 유산슬, 팔보채"
    #words_list = ['빵', '초코우유', '라떼', '소고기', '탕후루', '자몽에이드', '아메리카노', '돼지고기', '짬뽕', '떡볶이']
    
    words_list=[]
    for i in range(len(entire_Tag_list)):
        words_list.append(str(entire_Tag_list[i]).split('-')[0][0:-1])
    print(words_list)
    
    #recent_search_word = str(recent_search_word).split('-')[1][0:-1]
    recent_search_word = recent_search_word[0]
